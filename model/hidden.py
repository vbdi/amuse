...
class Hidden:
    ...
    def train_on_batch(self, batch: list):
        ...
        with torch.enable_grad():
            ...
            #print(decoded_messages, messages)
            g_loss_dec = self.mse_loss(decoded_messages, messages)
            
            #g_loss = self.config.adversarial_loss * g_loss_adv  + self.config.decoder_loss * g_loss_dec  +  self.config.encoder_loss * g_loss_enc
            #saeed
            g_loss = self.config.adversarial_loss * g_loss_adv +  self.config.encoder_loss * g_loss_enc + self.config.decoder_loss * g_loss_dec.clone()
    ...
    def hidden_encode(self, image: Image):

        image_tensor = ToPILImage()(image)

        return self.encoder(image_tensor)        

    def hidden_decode(self, image: Image):
