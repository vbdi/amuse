...
def save_watermarked(watermarked_images, target_folder, filenames, resize_to=None):
    watermarked_images = watermarked_images[:watermarked_images.shape[0], :, :, :].cpu()
    ...
    watermarked_images = watermarked_images.permute(0, 2, 3, 1).cpu().numpy()
    for i in range(watermarked_images.shape[0]):
            image = np.clip(watermarked_images[i]*255.0, 0, 255).astype(np.uint8)
            im = Image.fromarray(image)
            original_dir = filenames[i]
            img_name = os.path.basename(original_dir)
            fp = os.path.join(target_folder, img_name)
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions, dataset_name = None):
    ...
    if dataset_name is None: 
        ...
        data_transforms = {
            ...
            'test': transforms.Compose([
                transforms.ToTensor(),
                transforms.Resize((hidden_config.H,hidden_config.W),interpolation=PIL.Image.BICUBIC),
                transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
            ])
        }
        train_images = datasets.Flowers102(train_options.train_folder, 'train', transform = data_transforms['test'],download = True) #train split of flower watermarking method watermarked with Hidden test configs
        train_loader = torch.utils.data.DataLoader(train_images, batch_size=train_options.batch_size, shuffle=False, num_workers=4)
        validation_loader = None
        # val_images = datasets.Flowers102(train_options.train_folder, 'val', transform = data_transforms['test'],download = True) #train split of flower watermarking method watermarked with Hidden test configs
        # validation_loader = torch.utils.data.DataLoader(val_images, batch_size=train_options.batch_size, shuffle=False, num_workers=4)

        # test_images = datasets.Flowers102(train_options.train_folder, 'test', transform = data_transforms['test'],download = True) #train split of flower watermarking method watermarked with Hidden test configs
        # test_loader = torch.utils.data.DataLoader(test_images, batch_size=train_options.batch_size, shuffle=False, num_workers=4)

    return train_loader, validation_loader

def get_data_loaders_for_orig(hidden_config: HiDDenConfiguration, train_options: TrainingOptions, dataset_name = None):
    """ Get torch data loaders for training and validation. The data loaders take a crop of the image,
    transform it into tensor, and normalize it."""
    ...
    if dataset_name is None: 
        ...
        validation_images = datasets.ImageFolder(train_options.validation_folder, data_transforms['test'])
        val_loader = torch.utils.data.DataLoader(validation_images, batch_size=train_options.batch_size,
                                                        shuffle=False, num_workers=4)
        test_loader = None
        ...
        val_images = datasets.Flowers102(train_options.train_folder, 'val', transform = data_transforms['test'],download = True) #train split of flower watermarking method watermarked with Hidden test configs
        val_loader = torch.utils.data.DataLoader(val_images, batch_size=train_options.batch_size, shuffle=False, num_workers=4)
